CodaUtility = {}
CodaUtility.page_cycler_values = {}

G.FUNCS.coda_default = function(e)
    local args = e.config.pass_through
    local page_from = CodaUtility.page_cycler_values[args.key].page
    local page_to = CodaUtility.page_cycler_values[args.key].page + e.config.direction
    if page_to == 0 then page_to = args.total_pages
    elseif page_to > args.total_pages then page_to = 1 end
    CodaUtility.page_cycler_values[args.key].page = page_to
    CodaUtility.page_cycler_values[args.key].text = localize('k_page')..' '..CodaUtility.page_cycler_values[args.key].page..'/'..args.total_pages
    
    
    if e.config.switch_func and type(e.config.switch_func) == 'function' then
        e.config.switch_func({from = page_from, to = page_to})
    else
        sendInfoMessage('No switch_func provided', 'CodaUtility')
        sendInfoMessage(tprint({from = page_from, to = page_to}), 'CodaUtility')
    end
end

function CodaUtility.page_cycler(args)
    args = args or {}
    ...
    args.button = args.button or 'coda_default'
    ...
    if not args.page_label then
        CodaUtility.page_cycler_values[args.key] = {page = 1}
        CodaUtility.page_cycler_values[args.key].text = localize('k_page')..' '..CodaUtility.page_cycler_values[args.key].page..'/'..args.total_pages
        args.page_label = CodaUtility.page_cycler_values[args.key]
    end 

    local cycler = {n=G.UIT.R, config = {align = 'cm', minh = args.h or nil}, nodes = {
        {n=G.UIT.C, config={pass_through = args, switch_func = args.switch_func, r = 0.1, colour = args.colour, minw = args.button_w * args.scale, align = 'tm', shadow = args.shadow, direction = -1, button = args.button, hover = args.hover, minh = 0.5}, nodes = {
            {n=G.UIT.T, config = {text = args.left, scale = args.scale, colour = args.button_colour}}
        }},
        {n=G.UIT.C, config = {align = 'cm', minw = args.w * args.scale}, nodes = {
...
function CodaUtility.create_toggle(args)
    args = args or {}
